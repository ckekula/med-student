...
class LongCaseAttempt(Base):
    ...
    status: Mapped[AttemptStatus] = mapped_column(Enum(AttemptStatus), nullable=False, default=AttemptStatus.IN_PROGRESS_HISTORY, index=True)
...
class LongCaseAttemptMessage(Base):
    ...
    attempt_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("long_case_attempts.id", ondelete="CASCADE"), nullable=False, index=True)
...
class LongCaseAttemptExaminationLog(Base):
    ...
    examination_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("long_case_examinations.id"), index=True)
...
class LongCaseAttemptHistoryResult(Base):
    ...
    __table_args__ = (UniqueConstraint("attempt_id", "history_item_id", name="uq_attempt_checklist_item"),)

    attempt_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("long_case_attempts.id", ondelete="CASCADE"), nullable=False, index=True)
    history_item_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("history_items.id", ondelete="CASCADE"), nullable=False, index=True)

    is_met: Mapped[bool] = mapped_column(Boolean, nullable=False)
    score_awarded: Mapped[float] = mapped_column(Float, nullable=False)
    evaluation_method_used: Mapped[EvaluationMethod] = mapped_column(Enum(EvaluationMethod), nullable=False)
